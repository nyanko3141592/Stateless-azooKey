"""Reproduce frozen-v5 comparisons and enforce v6 development release floors."""
import json
import os
...
out = Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else root / "LocalModel/accuracy-v6/recheck"
...
for name, fixture in fixtures.items():
    ...
    env.update(LOCAL_ACCURACY_BASELINE="2108a60", LOCAL_ACCURACY_CASES="LocalModel/" + fixture,
    ...
    stats = {}
    for version in ["2108a60", "current"]:
...
assert summary["fresh48"]["current"]["exact"] >= 44
assert summary["fresh48"]["current"]["damage"] <= 73
for name, stats in summary.items():
    assert stats["current"]["damage"] <= stats["2108a60"]["damage"], name
print("V6 goals passed. All sets are now development regressions; not new held-out evidence.")
