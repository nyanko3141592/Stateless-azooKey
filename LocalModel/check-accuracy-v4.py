"""Reproduce frozen-v3 comparisons and enforce v4 development release floors."""
import json
import os
...
out = Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else root / "LocalModel/accuracy-v4/recheck"
...
for name, fixture in fixtures.items():
    ...
    env.update(LOCAL_ACCURACY_BASELINE="781996f", LOCAL_ACCURACY_CASES="LocalModel/" + fixture,
    ...
    stats = {}
    for version in ["781996f", "current"]:
...
assert summary["prior80"]["current"]["exact"] >= 79
assert summary["prior80"]["current"]["damage"] <= 80
assert summary["final40"]["current"]["exact"] >= 36
print("V4 goals passed. All sets are now development regressions; not new held-out evidence.")
