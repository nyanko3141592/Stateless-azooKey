...
`Stateless-azooKey Video.app` は録画専用の表示です。通常の入力メソッドとデモアプリの画面は変更しません。
...
動画専用アプリは `~/Applications/Stateless-azooKey Video.app`。「デモを再生」で4例を実演します。インストーラはこのアプリだけを更新し、ネットワーク entitlement を付けません。
